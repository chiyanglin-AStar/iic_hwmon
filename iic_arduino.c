...
static struct i2c_board_info __initdata iic_board[] =
{
        {
                I2C_BOARD_INFO("arduino", 0x49),
        },
        {
...
initialization of ‘int (*)(struct inode *, struct file *)’ from incompatible pointer type ‘ssize_t (*)(struct inode *, struct file *)’ {aka ‘long int (*)(struct inode *, struct file *)’} [-Werror=incompatible-pointer-types]
...
static int dummy_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    i2c_register_board_info(0, iic_board, ARRAY_SIZE(iic_board));
    ...
    if (err) {
        ...
        printk("printk:Dummy device is registerred successfully.\n");
    }
    ...
    if (!device) {
      ...
      printk("printk:create device entry under sysfs successfully.\n");
    }
